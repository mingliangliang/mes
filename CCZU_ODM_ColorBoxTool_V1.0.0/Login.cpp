...
void CLogin::OnChangeEditOrder() 
{
	...
	if(m_csOrder.GetLength() != 0)
	{
		...
		if(m_iKeyPeriod > 100)
		{
			MessageBox("Please use the scan gun input!");
			m_csOrder.Empty();
			UpdateData(FALSE);
			m_ctrOrder.SetFocus();
			return;
...
void CLogin::OnOK() 
{
	...
	{
		MessageBox("Order number or line can not be empty!");
		m_ctrOrder.SetFocus();
		return;
	}
	/*
...
void CLogin::OnCheckSelect() 
{
	if (m_ctrSelect.GetCheck() == TRUE)
	{
		...
		{
			MessageBox("Failed to get a list of items!");
			return;
		}
...
void CLogin::OnCloseupComboProduct() 
{
	...
	{
		MessageBox("Failed to get the order list!");
		return;
	}
...
bool CLogin::GetOrderList()
{
	...
	try
	{
		...
		while(!((CColorBoxApp *)AfxGetApp())->m_pRst->adoEOF)
		{			
			var = ((CColorBoxApp *)AfxGetApp())->m_pRst->GetCollect("OrderName"); 
			temp=(LPCTSTR)(_bstr_t)var;
