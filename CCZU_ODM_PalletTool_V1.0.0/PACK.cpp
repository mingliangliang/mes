...
BOOL CPACKApp::InitInstance()
{
	...
	//检查进程是否已经运行
	HANDLE dlgHandle = ::CreateMutex(NULL, FALSE, _T("DEWAV_ODM_PalletTool.exe"));
	...
	if (LoginDlg.DoModal() == IDOK)
	{
		...
		pDocTemplate = new CSingleDocTemplate(
			IDR_MAINFRAME,
			RUNTIME_CLASS(CPACKDoc),
			RUNTIME_CLASS(CMainFrame),       // main SDI frame window
			RUNTIME_CLASS(CPACKView));
		...
		m_pMainWnd->SetWindowText("CCZU ODM MES System-----Pallet Package Tool");
...
/////////////////////////////////////////////////////////////////////////////
// CPACKApp message handlers

BOOL CPACKApp::InitADO()
{
	CoInitialize(NULL);	
	HRESULT hrpC,hrpR;
...
bool CPACKApp::ConnectMDBDatabase()
{
	CString temp;
	_variant_t var;
	CString sql;
...
BOOL CPACKApp::CheckToolVersion()
{
	CString     csVer;
	CString     strVersion ;
	CString     sql;
	...
	try
	{
		...
		if(!m_pRst->adoEOF)
		{
			var = m_pRst->GetCollect("PalletTool");
