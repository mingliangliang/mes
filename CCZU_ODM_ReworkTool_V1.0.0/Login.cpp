...
void CLogin::OnChangeEditOrder() 
{
	...
	if(m_csOrder.GetLength() != 0)
	{
		...
		if(m_iKeyPeriod > 100)
		{
			MessageBox("Please use the scanner input!");
			m_csOrder.Empty();
			UpdateData(FALSE);
			m_ctrOrder.SetFocus();
			return;
	...
	if(m_csOrder.Right(2) == "\r\n")
	{
		m_csOrder.Replace("\r\n","");
		UpdateData(FALSE);
		m_ctrComboxType.EnableWindow(TRUE);
		m_ctrComboxType.SetFocus();
	}
}
...
void CLogin::OnOK() 
{
	...
	if (m_csOrder == "")
	{
		MessageBox("Order can not be NULL!");
		m_ctrOrder.SetFocus();
		return;
	}
	...
	if (m_iComboxType == -1)
	{
		MessageBox("Please select the type of sampling!");
		m_ctrComboxType.SetFocus();
		return;
	}
