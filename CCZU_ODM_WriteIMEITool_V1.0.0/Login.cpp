...
void CLogin::OnChangeEditOrder() 
{
	...
	if(m_csOrder.GetLength() != 0)
	{
		...
		if(m_iKeyPeriod > 100)
		{
			MessageBox("Please Use The Scanner!");
			m_csOrder.Empty();
			UpdateData(FALSE);
			m_ctrOrder.SetFocus();
			return;
...
void CLogin::OnOK() 
{
	...
	{
		MessageBox("Order Number OR Line Can't Be Empty!");
		m_ctrOrder.SetFocus();
		return;
	}
...
bool CLogin::GetOrderList()
{
	...
	try
	{
		...
		while(!((CFullWriteApp *)AfxGetApp())->m_pRst->adoEOF)
		{			
			var = ((CFullWriteApp *)AfxGetApp())->m_pRst->GetCollect("OrderName"); 
			temp=(LPCTSTR)(_bstr_t)var;
