...
void CConfig::InitConfigPath()
{
	...
	this->m_csResourcePath=Temp;
	this->m_csLogPath=Temp;
	this->m_iniFile.m_csIniFileName += "\\config.ini";
	this->m_csResourcePath += "\\label.txt";
	this->m_csLogPath += "\\Log\\";
}
...
int CConfig::GetList(char *cs, char List[200][30])
{
	char crEachData[30];
	int i = 0;
	int iNum = 0;
	
	memset(crEachData, 0, sizeof(crEachData));
	
	while(*cs != '\0')
	{
		if(*cs == ',')
		{
			memset(List[iNum], 0, sizeof(List[iNum]));
			crEachData[i] = '\0';
			strcpy(List[iNum], crEachData);
			i = 0;
			iNum++;
			memset(crEachData, 0, sizeof(crEachData));
		}
