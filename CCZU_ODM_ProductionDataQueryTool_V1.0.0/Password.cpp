...
void CPassword::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_PSW, m_ctrPassword);
	DDX_Text(pDX, IDC_EDIT_PSW, m_csPassword);
...
void CPassword::OnOK() 
{
	...
	try
	{
		...
		{
			//数据库内有记录
			var = ((CPACKApp *)AfxGetApp())->m_pRst->GetCollect("PermissionLevel");
			ileveltemp=var.intVal;
			
			var = ((CPACKApp *)AfxGetApp())->m_pRst->GetCollect("ProductionDataQueryTool");
			iTooltemp1=var.intVal;

			var = ((CPACKApp *)AfxGetApp())->m_pRst->GetCollect("AllTool");
